                                        ...
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                            ...
                                            @for($i = 0; $i < $users->lastPage(); $i++)
                                                @if ($users->currentPage() === $i+1)
                                                    <a href="{{ $users->url($i+1) }}" aria-current="page" class="z-10 bg-indigo-50 border-indigo-500 text-indigo-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium"> {{ $i+1 }} </a>
                                                @else
                                                    <a href="{{ $users->url($i+1) }}" class="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium"> {{ $i+1 }} </a>
                                                @endif
                                            @endfor
                                            <!-- Button Next -->
                                            @if($users->onLastPage())
